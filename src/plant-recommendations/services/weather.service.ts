...
import { safeLinearRegression } from "../../utils/math/linear-regression/linear-regression";
import {
...
export function predictFutureWeather(
  ...
  if (!historicalData?.T2M || !historicalData?.PRECTOTCORR) {
    throw new Error(
      "Invalid historical data format: Missing T2M or PRECTOTCORR"
    );
  }
  ...
  const temperatureData = filterDataByMonth(
    historicalData.T2M,
    startMonth,
    durationMonths
  );
  const precipitationData = filterDataByMonth(
    historicalData.PRECTOTCORR,
    startMonth,
    durationMonths
  );
  ...
  if (
    ...
    Object.keys(precipitationData).length === 0
  ) {
  ...
  const precipRegression = safeLinearRegression(
    precipitationYears,
    precipitations
  );
  ...
  const predictedTemperature =
    tempRegression.slope * (lastTemperatureYear + 1) + tempRegression.intercept;
  const predictedPrecipitation =
    precipRegression.slope * (lastPrecipitationYear + 1) +
    precipRegression.intercept;
